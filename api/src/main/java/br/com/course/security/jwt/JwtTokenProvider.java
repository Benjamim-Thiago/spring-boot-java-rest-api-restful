package br.com.course.security.jwt;

import java.util.Base64;
import java.util.Date;
import java.util.List;

import br.com.course.Validation.AccessDeniedException;
...
public class JwtTokenProvider {
	...
	@Value("${security.jwt.token.expire-access-token-length:3600000}")
	private long validityAccessTokenInMilliseconds = 3600000; // 1h

	@Value("${security.jwt.token.expire-refresh-token-length:10800000}")
	private long validityRefreshTokenInMilliseconds = 10800000; // 3h
	...
	private String getRefreshToken(String username, List<String> roles, Date now) {
		Date validityRefreshToken = new Date(now.getTime() + validityRefreshTokenInMilliseconds);
		return JWT.create()
				.withClaim("roles", roles)
				.withIssuedAt(now)
